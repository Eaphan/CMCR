...
class ComposeClouds:
    ...
    def __call__(self, pc, pos_non_manifold=None):
        for transform in self.transforms:
            pc, pos_non_manifold = transform(pc, pos_non_manifold)
        return pc, pos_non_manifold
...
class Rotation_z:
    ...
    def __call__(self, pc, pos_non_manifold=None):
        ...
        pc = pc @ R.T
        if pos_non_manifold is not None:
            pos_non_manifold = pos_non_manifold @ R.T
        return pc, pos_non_manifold
    ...
    def __call__(self, pc, pos_non_manifold=None):
        for curr_ax in range(2):
            if random.random() < self.p:
                ...
                if pos_non_manifold is not None:
                    pos_non_manifold[:, curr_ax] = -pos_non_manifold[:, curr_ax]
        return pc, pos_non_manifold
...
class ComposeAsymmetrical:
    ...
    def __call__(self, pc, features, img, ori_img, pairing_points, pairing_images, superpixels=None):
        for transform in self.transforms:
            pc, features, img, ori_img, pairing_points, pairing_images, superpixels = transform(
                pc, features, img, ori_img, pairing_points, pairing_images, superpixels
            )
        if superpixels is None:
            return pc, features, img, ori_img, pairing_points, pairing_images
        return pc, features, img, ori_img, pairing_points, pairing_images, superpixels
...
class ResizedCrop:
    ...
    def __call__(self, pc, features, images, ori_images,  pairing_points, pairing_images, superpixels):
        imgs = torch.empty(
            (images.shape[0], 3) + tuple(self.crop_size), dtype=torch.float32
        )
        ori_imgs = torch.empty(
            (ori_images.shape[0], 3) + tuple(self.crop_size), dtype=torch.float32
        )
        ...
        if self.crop_center:
            ...
            for id, img in enumerate(images):
                ...
                ori_imgs[id] = resize(ori_images[id], self.crop_size, self.img_interpolation)
                if superpixels is not None:
                    sps[id] = resize(
            ...
            for id, img in enumerate(images):
                ...
                ori_imgs[id] = resized_crop(
                    ori_images[id], i, j, h, w, self.crop_size, self.img_interpolation
                )
                if superpixels is not None:
                    sps[id] = resized_crop(
                        superpixels[id],
        ...
        if superpixels is None:
            return pc, features, imgs, ori_imgs, pairing_points_out, pairing_images_out, superpixels
        return pc, features, imgs, ori_imgs, pairing_points_out, pairing_images_out, sps
...
class FlipHorizontal:
    ...
    def __call__(self, pc, features, images, ori_images, pairing_points, pairing_images, superpixels):
        ...
        for i, img in enumerate(images):
            if random.random() < self.p:
                images[i] = hflip(img)
                ori_images[i] = hflip(ori_images[i])
                if superpixels is not None:
                    superpixels[i] = hflip(superpixels[i: i + 1])
                mask = pairing_images[:, 0] == i
                pairing_images[mask, 2] = w - 1 - pairing_images[mask, 2]

        return pc, features, images, ori_images, pairing_points, pairing_images, superpixels
